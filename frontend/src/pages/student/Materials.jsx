...
const Materials = () => {
  ...
  useEffect(() => { fetchMaterials(); }, [search, typeFilter]);
        ...
        {loading ? <div className="loading"><FileText size={20} /> Loading materials...</div> :
          materials.length === 0 ? (
            <div className="empty-state">
              <FileText size={60} />
              <h3 style={{ marginTop: 16 }}>No materials found</h3>
            </div>
          ) : (
            <div style={{ display: 'grid', gap: 12 }}>
              {materials.map(mat => (
                <div key={mat._id} className="card" style={{ padding: 18, display: 'flex', alignItems: 'center', gap: 16 }}>
                  <div style={{ width: 52, height: 52, background: 'var(--bg-card2)', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, flexShrink: 0 }}>
                    {TYPE_ICONS[mat.type] || '📁'}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 600, fontSize: 15, marginBottom: 4 }}>{mat.title}</div>
                    ...
                    {mat.description && <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 4 }}>{mat.description}</p>}
                    {mat.tags?.length > 0 && (
            ...
            <div className="modal" onClick={e => e.stopPropagation()}>
              ...
              <form onSubmit={handleUpload}>
                ...
                <div onClick={() => fileRef.current.click()} style={{
                  ...
                  cursor: 'pointer', marginBottom: 16, background: file ? 'rgba(79,70,229,0.05)' : 'transparent',
                  transition: 'all 0.2s'
                }}>
                  ...
                  <File size={32} color={file ? 'var(--primary)' : 'var(--text-faint)'} style={{ margin: '0 auto 8px' }} />
                  <div style={{ fontWeight: 600, fontSize: 14 }}>{file ? file.name : 'Click to select file'}</div>
                  <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>PDF, DOC, PPT, XLS, Images, Videos (max 50MB)</div>
                </div>
                ...
                <div className="form-group" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <label htmlFor="isPublic" style={{ margin: 0 }}>Make publicly accessible to all students</label>
                </div>
